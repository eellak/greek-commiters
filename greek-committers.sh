...
do
    curl -s 'https://api.github.com/search/users?per_page=100&q=followers:%3E10+location:'$loca
    curl -s 'https://api.github.com/search/users?per_page=100&q=repos:%3E5+location:'$loca
done |
sed -n 's/.*"login": "\(.*\)",/\1/p' |
sort -u >users.txt
